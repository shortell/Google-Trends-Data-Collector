...
class Today(Resource):
    def get(self):
        ...
        data_response = []
        data = get_today_data(country_id, start, end, rank_min, rank_max, limit, offset)
        if data is None:
            return jsonify(None)
        counts = get_today_count_and_unique_count(country_id, start, end, rank_min, rank_max, limit)
        for record in data:
